...
public class RefuelMission : Mission
{
    protected readonly BoatController boatController;
    protected TargetInformation currentTarget;

    public RefuelMission(BoatController boatController) : this("Refuel", boatController) { }
    public RefuelMission(string name, BoatController boatController) : base(name)
    {
        this.boatController = boatController;
    }
    public override float GetPriority()
    {
        float priority = 0.75f * Math.Pow(1 - boatController.EnergyFraction, 2);
    ...
    protected void UpdateTarget()
    {
        ...
        foreach(TargetInformation target in boatController.targets.Where(target => target.Name == "Energy Powerup"))
        {
