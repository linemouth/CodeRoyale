...
void SimpleNoiseMotion_float(float2 position, float2 velocity, float density, float amplitude, out float result)
{
	float2 uv = velocity * _Time + position * density;
	result = GeneratePerlinNoise2D(uv) * amplitude;
}
float CompositePerlinNoise_float(float2 position, float2 velocity1, float2 velocity2, float2 velocity3, float2 velocity4, float density1, float density2, float density3, float density4, float amplitude1, float amplitude2, float amplitude3, float amplitude4, out float offset)
{
    float uv1 = velocity1 * _Time + position * density1;
    float uv2 = velocity2 * _Time + position * density2;
    float uv3 = velocity3 * _Time + position * density3;
    float uv4 = velocity4 * _Time + position * density4;
    offset = -0.5f * (amplitude1 + amplitude2 + amplitude3 + amplitude4);
    offset += GeneratePerlinNoise2D(uv1) * amplitude1;
    offset += GeneratePerlinNoise2D(uv2) * amplitude2;
    offset += GeneratePerlinNoise2D(uv3) * amplitude3;
    offset += GeneratePerlinNoise2D(uv4) * amplitude4;
    return offset;
}
float3 CompositeTextures_float(float2 position, float2 velocity1, float2 velocity2, float2 velocity3, float2 velocity4, float density1, float density2, float density3, float density4, sampler2D sampler1, sampler2D sampler2, sampler2D sampler3, sampler2D sampler4, out float3 normal)
{
    float2 uv1 = velocity1 * _Time + position * density1;
    float2 uv2 = velocity2 * _Time + position * density2;
    float2 uv3 = velocity3 * _Time + position * density3;
    float2 uv4 = velocity4 * _Time + position * density4;
    normal = tex2D(sampler1, uv1);
    normal += tex2D(sampler2, uv2);
    normal += tex2D(sampler3, uv3);
    normal += tex2D(sampler4, uv4);
    normal = normalize(float3(normal));
    return normal;
}
float3 GetNormalFromHeight_float(float height, out float3 normal)
{
    normal = normalize(float3(ddx(height), ddy(height), 1.0));
    return normal;
}
